...
const createTokens = async (user, accessTokenKey, refreshTokenKey) => {
    const accessToken = await JWT_1.default.encode(new JWT_1.JwtPayload(config_1.tokenInfo.issuer, config_1.tokenInfo.audience, user.userId.toString(), accessTokenKey, config_1.tokenInfo.accessTokenValidityDays));
    if (!accessToken)
        throw new ApiError_1.InternalError();
    const refreshToken = await JWT_1.default.encode(new JWT_1.JwtPayload(config_1.tokenInfo.issuer, config_1.tokenInfo.audience, user.userId.toString(), refreshTokenKey, config_1.tokenInfo.refreshTokenValidityDays));
    if (!refreshToken)
        throw new ApiError_1.InternalError();
    return {
